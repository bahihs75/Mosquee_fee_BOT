...
STATUS_LABELS = {
    ...
    Status.APPROVED_BY_ADMIN.value: "اعتمدتها الإدارة – بانتظار الصندوق",
...
def money(value: Any, currency: str = "DZD") -> str:
    return f"{Decimal(value):,.2f} {escape(currency)}"
...
def request_summary(row: dict[str, Any], include_actions: bool = False) -> str:
    ...
    text = (
        ...
        f"<b>المدة:</b> {escape(str(row.get('duration_text', '')))}\n"
        f"<b>المبلغ:</b> {money(row.get('amount_requested', 0), row.get('currency', 'DZD'))}\n"
